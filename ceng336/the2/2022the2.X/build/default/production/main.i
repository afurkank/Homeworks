...
timer1_interrupt:
    ; used to clear ((PORTC) and 0FFh), 0, a and ((PORTC) and 0FFh), 1, a pins
...
main:
    ...
    ; by setting ((INTCON) and 0FFh), 7, a and ((INTCON) and 0FFh), 6, a bits to 1
    bsf INTCON, 7
    bsf INTCON, 6
...
timer0_interrupt_occurred:
    ...
    ; Also, ((PORTC) and 0FFh), 0, a is set to 1 at the start of each beat
    ; and ((PORTC) and 0FFh), 1, a is set to 1 at the start of each bar.
    ; ((PORTC) and 0FFh), 0, a and ((PORTC) and 0FFh), 1, a are cleared with Timer1 interrupt.
    ; We enable Timer1 here so that after 50ms
    ; it clears ((PORTC) and 0FFh), 0, a and ((PORTC) and 0FFh), 1, a
    ...
    call start_timer1 ; clear ((PORTC) and 0FFh), 0, a and ((PORTC) and 0FFh), 1, a
...
light_rc0_and_rc1:
    movlw 00000011B
    movwf LATC ; light up ((PORTC) and 0FFh), 0, a and ((PORTC) and 0FFh), 1, a
    return
light_rc0:
    movlw 00000001B
    movwf LATC ; light up ((PORTC) and 0FFh), 0, a
    return

;############################## TIMER1 INTERRUPT ###############################

timer1_interrupt_occurred:
    ; interrupt to clear ((PORTC) and 0FFh), 0, a and ((PORTC) and 0FFh), 1, a
    ; interrupt happens everytime Timer1 overflows
    ; Timer1 overflows in 50ms
    movlw 00000000B
    movwf LATC ; clear both ((PORTC) and 0FFh), 0, a and ((PORTC) and 0FFh), 1, a
    return
...
rb_interrupt_occurred:
    ...
    movff last_rb4, WREG ; WREG = previous ((PORTB) and 0FFh), 4, a
    cpfseq current_rb4 ; if current and previous ((PORTB) and 0FFh), 4, a is same, then skip
    call rb4_changed ; else, check if the change is a click
    movff last_rb5, WREG ; WREG = previous ((PORTB) and 0FFh), 5, a
    cpfseq current_rb5 ; if current and previous ((PORTB) and 0FFh), 5, a is same, then skip
    call rb5_changed ; else, check if the change is a click
    movff last_rb6, WREG ; WREG = previous ((PORTB) and 0FFh), 6, a
    cpfseq current_rb6 ; if current and previous ((PORTB) and 0FFh), 6, a is same, then skip
    call rb6_changed ; else, check if the change is a click
    movff last_rb7, WREG ; WREG = previous ((PORTB) and 0FFh), 7, a
    cpfseq current_rb7 ; if current and previous ((PORTB) and 0FFh), 7, a is same, then skip
...
rb4_changed:
    ...
    call rb4_pressed ; else, ((PORTB) and 0FFh), 4, a is clicked, take appropriate action
    return
rb5_changed:
    cpfsgt current_rb5 ; skip if current_rb5 > last_rb5 (button change isn't a click)
    call rb5_pressed ; else, ((PORTB) and 0FFh), 5, a is clicked, take appropriate action
    return
rb6_changed:
    cpfsgt current_rb6 ; skip if current_rb6 > last_rb6 (button change isn't a click)
    call rb6_pressed ; else, ((PORTB) and 0FFh), 6, a is clicked, take appropriate action
    return
rb7_changed:
    cpfsgt current_rb7 ; skip if current_rb7 > last_rb7 (button change isn't a click)
    call rb7_pressed ; else, ((PORTB) and 0FFh), 7, a is clicked, take appropriate action
    return
...
start_timer2:
    clrf TMR2
    ; Bits 6-3 ((T2CON) and 0FFh), 6, a:((T2CON) and 0FFh), 3, a = 0000, no postscaling.
    ; Bit 2 ((T2CON) and 0FFh), 2, a = 1, enable timer2 now.
    ; Bits 1-0 ((T2CON) and 0FFh), 1, a:((T2CON) and 0FFh), 0, a = 01, 1:4 prescaling.
    movlw 00000101B
    movwf T2CON
    return

; This is the timer for clearing ((PORTC) and 0FFh), 0, a and ((PORTC) and 0FFh), 1, a
...
start_timer1:
    ...
    ; Bit 6 ((T1CON) and 0FFh), 6, a = 1, device clock is derived from Timer1 oscillator.
    ; Bit 5-4 ((T1CON) and 0FFh), 5, a-((T1CON) and 0FFh), 4, a = 11, 1:8 prescaling.
    ; Bit 3 ((T1CON) and 0FFh), 3, a = 1, oscillator is enabled.
    ; Bit 2 T1SYNC0 = 0, this bit is ignored.
    ; Bit 1 ((T1CON) and 0FFh), 1, a = 0, use internal clock.
    ; Bit 0 ((T1CON) and 0FFh), 0, a = 1, enable Timer1 now.
    movlw 00110001B
    movwf T1CON
    return
...
start_timer0:
    ...
    ; Bit 6 ((T0CON) and 0FFh), 6, a = 0, 16 bit mode.
    ; Bit 5 ((T0CON) and 0FFh), 5, a = 0, use internal instruction cycle clock.
    ; Bit 4 ((T0CON) and 0FFh), 4, a = 0, increment on low-to-high transition on ((PORTA) and 0FFh), 4, a pin.
    ; Bit 3 ((T0CON) and 0FFh), 3, a = 0, enable prescaling.
    ; Bit 2-0 ((T0CON) and 0FFh), 2, a-((T0CON) and 0FFh), 0, a = 011, 1:16 prescaling.
    movlw 10000011B
    movwf T0CON
    return
