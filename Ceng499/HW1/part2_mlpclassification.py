...
def forward_pass(w1, b1, w2, b2, input_data):
    ...
    probs = torch.softmax(final_layer, dim=-1)
    return probs
...
w1 = torch.normal(0, 1, (3, 16), requires_grad=True)
...
b1 = torch.normal(0, 1, (16,), requires_grad=True)
...
w2 = torch.normal(0, 1, (16, 3), requires_grad=True)
...
b2 = torch.normal(0, 1, (3,), requires_grad=True)
...
for iteration in range(1, ITERATION+1):
    ...
    train_mean_crossentropy_loss = -torch.sum(torch.mul(train_label, torch.log(train_predictions)), dim=-1).mean()
    ...
    with torch.no_grad():
        ...
        train_accuracy = (torch.sum(train_predictions.argmax(dim=-1) == train_label.argmax(dim=-1)) / train_label.shape[0]) * 100
        ...
        validation_mean_crossentropy_loss = -torch.sum(torch.mul(validation_label, torch.log(validation_predictions)), dim=-1).mean()
        ...
        validation_accuracy = (torch.sum(validation_predictions.argmax(dim=-1) == validation_label.argmax(dim=-1)) / validation_label.shape[0]) * 100
...
with torch.no_grad():
    ...
    test_accuracy = (torch.sum(test_predictions.argmax(dim=-1) == test_label.argmax(dim=-1)) / test_label.shape[0]) * 100
