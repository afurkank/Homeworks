...
public class SimulationRunner {
    ...
    private static void stepAllEntities() {
        ...
        int entityDiameter = Common.getEntityDiameter();
        for (int i = 0; i < Common.getCustomerNo(); i++) {
            Customer c = customerList.get(i);
            if (c.getPosition().getIntX() < -2 * entityDiameter || c.getPosition().getIntX() > Common.getWindowWidth()
                    ||
                    c.getPosition().getIntY() < -2 * entityDiameter
